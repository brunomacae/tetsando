...
const StyledFarmStakingCard = styled(Card)`
  min-height: 376px;
  background: ${({ theme }) => theme.isDark && '#e66f0e'};
`
...
const Label = styled(Text).attrs({ color: 'primary' })`
  font-size: 14px;
  color: ${({ theme }) => theme.isDark && '#e66f0e'};
`
  ...
  return (
    <StyledFarmStakingCard>
      <CardBody>
        ...
        <Block>
          <Label>{TranslateString(544, 'SCARE to Harvest')}:</Label>
          <PefiHarvestBalance />
        </Block>
        <Block>
          <Label>{TranslateString(546, 'SCARE in Wallet')}:</Label>
          <PefiWalletBalance />
        </Block>
        <Actions>
          {account ? (
            ...
            >
              {pendingTx
                ? TranslateString(548, 'Collecting SCARE')
